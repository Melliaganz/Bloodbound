...
const Navbar: React.FC = () => {
  ...
  const currentLangDisplay = i18n.language.split('-')[0].toUpperCase();
    ...
    <header className="fixed top-0 w-full z-50 border-b border-white/5 bg-background-dark/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3 group cursor-pointer">
          <div className="size-8 flex items-center justify-center bg-primary rounded-sm rotate-45 group-hover:bg-accent-blood transition-colors duration-500">
            <span className="material-symbols-outlined -rotate-45 text-white text-xl">landscape</span>
          </div>
        ...
        <div className="flex items-center gap-6">
          ...
          >
            <span className={`${currentLangDisplay === 'FR' ? 'text-primary' : 'text-slate-500'}`}>FR</span>
            <span className="text-white/20">|</span>
            <span className={`${currentLangDisplay === 'EN' ? 'text-primary' : 'text-slate-500'}`}>EN</span>
          </button>
